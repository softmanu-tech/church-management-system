                ...
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="form-group col-md-6"  >
                        ...
                        <select class="form-control" required name="cluster_id">
                            ...
                            @foreach($getCluster as $value)
                              <option {{ (old('cluster_id', $getRecord->cluster_id) == $value->id) ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <div style="color:red">{{ $errors->first('date_of_birth' , $getRecord->date_of_birth) }}</div>
                    </div>
                    <div class="form-group col-md-6"  >
                        <label>Location<span style="color: red;">*</span></label>
                        <input type="text" class="form-control"  name="location" value="{{ old('location' , $getRecord->location) }}" required placeholder="Location">
                        <div style="color:red">{{ $errors->first('location') }}</div>
                    </div>
                    <div class="form-group col-md-6">
                        <label>Date Joined<span style="color: red;">*</span></label>
                        <input type="date" class="form-control"  name="join_date" value="{{ old('join_date' , $getRecord->join_date) }}" required>
                        <div style="color:red">{{ $errors->first('join_date') }}</div>
                    </div>
                    <div class="form-group col-md-6">
                        <label>Previous Altar<span style="color: red;">*</span></label>
                        <input type="text" class="form-control"  name="prev_altar" value="{{ old('prev_altar' , $getRecord->prev_altar) }}" required placeholder="Previous Altar">
                        <div style="color:red">{{ $errors->first('prev_altar') }}</div>
                    </div>
                    <div class="form-group col-md-6"  >
                        <label>Department<span style="color: red;"></span></label>
                        <input type="text" class="form-control"  name="department" value="{{ old('department' , $getRecord->department) }}" required placeholder="Department">
                    ...
                    <div class="form-group col-md-6"  >
                        ...
                        <select class="form-control"  name="nhif">
                            ...
                            <option  {{ (old('nhif' ,$getRecord->nhif) =='Yes') ? 'selected' : '' }} value="Yes">Yes</option>
                            <option  {{ (old('nhif' ,$getRecord->nhif) =='No') ? 'selected' : '' }} value="No">No</option>
                        </select>
